...
function MisMovimientos() {
  ...
  const fetchMovimientos = async () => {
    ...
    const token = localStorage.getItem("token");
    try {
      // --- Solo consulta los movimientos del empleado autenticado
      const res = await axios.get(
        "http://localhost:8080/api/movimientos-inventario/mios",
  ...
  const fetchProductos = async () => {
    const token = localStorage.getItem("token");
    try {
      const res = await axios.get("http://localhost:8080/api/productos/public", {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const handleSubmit = async (e) => {
    ...
    if (!form.producto_id || !form.cantidad || form.cantidad <= 0 || !form.tipo) {
...
Stock actual: ${producto.stock}, Cantidad a retirar: ${cantidad}`
  ...
  // Ordenamiento y paginación (igual que antes)
